	...
	public class F3DCameraTool : MonoBehaviour
	{
		...
		public void AutoSetup()
		{
			...
			if (!PrimaryCamera && !SecondaryCamera && Camera.allCameras.Length > 0)
			{
				if (Camera.allCameras != null)
				{
					...
					PrimaryCamera = result[0].transform;
					...
					{
						SecondaryCamera = result[1].transform;
			...
			if (Enabled && UnityEditor.SceneView.lastActiveSceneView != null)
			{
				...
				if (PrimaryCamera)
				{
					if (SetPrimaryCameraPosition)
						PrimaryCamera.position = UnityEditor.SceneView.lastActiveSceneView.camera.transform.position - UnityEditor.SceneView.lastActiveSceneView.camera.transform.forward * 0.1f;
					if (SetPrimaryCameraRotation)
						PrimaryCamera.rotation = UnityEditor.SceneView.lastActiveSceneView.camera.transform.rotation;
				}
				...
				if (SecondaryCamera)
				{
					if (SetSecondaryCameraPosition)
						SecondaryCamera.position = UnityEditor.SceneView.lastActiveSceneView.camera.transform.position - UnityEditor.SceneView.lastActiveSceneView.camera.transform.forward * 0.1f;
					if (SetSecondaryCameraRotation)
						SecondaryCamera.rotation = UnityEditor.SceneView.lastActiveSceneView.camera.transform.rotation;
				}
			}
		}
		...
		public void OnBecameActive()
		{
			if (AlignSceneCamOnStart && UnityEditor.SceneView.lastActiveSceneView != null && PrimaryCamera)
				UnityEditor.SceneView.lastActiveSceneView.LookAt(PrimaryCamera.position, PrimaryCamera.rotation, 0, false, true);
		}
		...
		public void OnBecameInactive()
		{
			...
			{
				PrimaryCamera.position = PrimaryCameraOriginPos;
				PrimaryCamera.rotation = PrimaryCameraOriginRot;
			}

			if (AlignSecondaryGameCamOnStop && SecondaryCamera)
			{
				SecondaryCamera.position = SecondaryCameraOriginPos;
				SecondaryCamera.rotation = SecondaryCameraOriginRot;
			}
		}
